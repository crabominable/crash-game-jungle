...
Bem-vindo ao projeto do Crash Game! Abaixo estão as instruções rápidas para rodar e testar o projeto na sua máquina.

## 🚀 Como Rodar o Projeto (Setup)

O projeto foi construído para subir de forma orquestrada e com zero configuração manual utilizando o Docker Compose.
Certifique-se de ter o Docker Desktop e o Bun instalados na sua máquina.
...
2. **Inicie toda a infraestrutura:**
   ...
   bun run docker:up
   ```
   *Este comando levanta os bancos (PostgreSQL), o RabbitMQ, o Keycloak (com os usuários já configurados), o Kong API Gateway e todos os serviços (`games` e `wallets`) em modo watch.*

3. **Inicie o Frontend:**
   Em um terminal separado (dentro da pasta raiz ou na pasta `frontend`), rode o ambiente de desenvolvimento Vite:
   ```bash
   cd frontend
   bun install
   bun run dev
   ...
   *Login padrão (Keycloak)*:
   - **User**: `player1`
   - **Password**: `password`

## 🧪 Como Rodar os Testes

O projeto conta com uma cobertura forte tanto nas regras de unidade (domínio de carteiras e jogos) quanto no fluxo de ponta a ponta.
...
- **Testes E2E (Fluxo Completo):**
  *Certifique-se de que a infraestrutura (`bun run docker:up`) esteja rodando para testar a mensageria e o gateway reais.*
  ```bash
  bun run test:e2e
  ```
...
As rotas RESTful possuem documentação automática e interativa nativa do NestJS:

- **API de Jogos (Games)**: [http://localhost:4001/api/docs](http://localhost:4001/api/docs)
- **API de Carteiras (Wallets)**: [http://localhost:4002/api/docs](http://localhost:4002/api/docs)

*O frontend consome essas APIs de forma roteada e ocultada através do Kong Gateway (porta `8000`).*

---

## Objetivo (Documento de Arquitetura e Decisões)
